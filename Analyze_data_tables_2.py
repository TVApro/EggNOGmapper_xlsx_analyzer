...
# НАЧАЛО
path_2 = '/media/lab330/Новый том/EggNOG/Tables_1_Total_number_of_genes_in_groups'
...
for j in output_tables_list:
    ...
    if db_control_1.empty == False:
        ...
        print('записан файл', 'arctic_'+j)
    else:
        print('Не обнаружено тенденций в базе', j, 'для арктических представителей')

    if db_control_2.empty == False:
        export(j, db_all, 'all_', path_output)
        print('записан файл', 'arctic_'+j)
    else:
        print('Не обнаружено тенденций в базе', j, 'для всех')
